...
for ii = 1:StepNum   
    ...
    CaptPoint(ii) = XInit(ii) + HorSpeed(ii)/sqrt(-grav/len);
    ...
    X0 = MX.sym('X0', nv);
    w = {w{:}, X0};
    lbw = [lbw; 0; 0; XInit(ii); 0; z_flight+len; 0];
    ubw = [ubw; 0; 0; XInit(ii); 0; z_flight+len; 0];
    w0 = [w0; 0; 0; 0; 0; 0; 0];
...
%             ubg = [ubg; 0];
        elseif k == N-1
            g = {g{:}, Xk - Xinit};
            lbg = [lbg; 0; 0; Xfinal(ii); HorSpeed(ii); 0; 0;];
            ubg = [ubg; 0; 0; Xfinal(ii); HorSpeed(ii); 0; 0;];
            g = {g{:}, Uk(1)};
            lbg = [lbg; 0];
            ubg = [ubg; 0];
        else
